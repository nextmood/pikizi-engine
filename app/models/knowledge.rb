...
class Knowledge < Root
  ...
  def cancel_recommendations(question, last_answer, quiz_instance, products)
    ...
    propagate_recommendations(question, choices_ok, quiz_instance.hash_pidurl_affinity, products, true)
  end
    ...
    choices_ok.each do |choice_ok|
      choice_ok.generate_hash_pidurl_weight(products).each do |pidurl, weight|
        if question.is_filter
          ...
          hash_pidurl_affinity[pidurl] += weight * (reverse_mode ? -1.0 : 1.0)
        end
      end
    end
  end
  ...
  def trigger_recommendations(quiz_instance, question, products, choices_ok, simulation)
    choice_idurls_ok = choices_ok.collect(&:idurl)
    answer = quiz_instance.record_answer(self.idurl, question.idurl, choice_idurls_ok)
    hash_pidurl_affinity = quiz_instance.hash_productidurl_affinity
...
class FeatureTags < Feature
  ...
  def range(among_products)
    ...
    min, max = among_products.inject([Set.new(all_tags_idurls), Set.new]) do |(intersection, union), product|

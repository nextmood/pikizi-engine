...
class InterpretorController < ApplicationController
  ...
  def add_opinion_to_review
    ...
    opinion = Opinion.const_get(params[:opinion_class]).create(:review_id => in_review.id, :paragraph_id => in_paragraph.id, :user_id => @current_user.id, :category => in_review.category)
